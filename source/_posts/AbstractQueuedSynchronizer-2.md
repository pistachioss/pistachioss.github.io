---
title: AQS源码分析2
date: 2020-05-14 23:18:46
categories: 多线程
tags: wiki
toc: true
description: "一行一行源码AbstractQueuedSynchronizer"

---

文章比较长，信息量比较大。文章标题是为了呼应前文，其实可以单独成文的，主要是希望看文章能系统看。
...
基本上本文把以上几点都说清楚了，看过[上一篇文章中对 AbstractQueuedSynchronizer 的介绍 ](/post/AbstractQueuedSynchronizer)，当然如果已经熟悉 AQS 中的独占锁了，那也可以直接看这篇。各小节之间基本上没什么关系，可以只关注自己感兴趣的部分。

其实这篇文章的信息量很大，至少**小时**才能看完，希望本文对得起大家的时间。
...
Tips: 这里重申一下，要看懂这个，必须要先看懂上一篇关于 [AbstractQueuedSynchronizer](/post/AbstractQueuedSynchronizer) 的介绍，或者你已经有相关的知识了，否则这节肯定是看不懂的。
...
我们常用 obj.wait()，obj.notify() 或 obj.notifyAll() 来实现相似的功能，但是，它们是基于对象的监视器锁的。需要深入了解这几个方法的读者，可以参考我的另一篇文章《[深入分析 java 8 编程语言规范：Threads and Locks](/post/Threads-And-Locks-md)》。而这里说的 Condition 是基于 ReentrantLock 实现的，而 ReentrantLock 是依赖于 AbstractQueuedSynchronizer 实现的。
...
## 再说 java线程中断和InterruptedException 异常
